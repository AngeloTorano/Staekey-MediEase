...
export default function Phase3FormsPage() {
  ...
  const [showPhase3Complete, setShowPhase3Complete] = useState(false)
  const [phase3Loading, setPhase3Loading] = useState(false)

  // NEW: Ongoing Phase 3 resume dialog states
  const [resumeDialogOpen, setResumeDialogOpen] = useState(false)
  const [resumeCheckLoading, setResumeCheckLoading] = useState(false)
  const [resumeSummary, setResumeSummary] = useState<{
    phase3_reg_id: number | null
    completeness: { registration: boolean; earScreening: boolean; aftercareAssessment: boolean; finalQC: boolean }
  ...
  // NEW: gating banners/dialogs
  const [needPhase3DialogOpen, setNeedPhase3DialogOpen] = useState(false)
  const [latestRegComplete, setLatestRegComplete] = useState(false)
  ...
  // CHANGE: add optional requirePhase3Reg to block non-registration sections until a P3 registration exists
  const ensurePatientAndRun = async (
    action?: () => Promise<void> | void,
    opts?: { requirePhase3Reg?: boolean },
  ) => {
    ...
    if (opts?.requirePhase3Reg && !phase3RegId) {
      setNeedPhase3DialogOpen(true)
      return
    }
    try {
      await action?.()
  ...
  const submitRegistrationSection = async () => {
    ...
    try {
      ...
      await loadPhase3Data(patientId) // refresh to attach incomplete registration
  ...
  const submitEarScreeningSection = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // ensure we link to the same P3 registration if it exists
        phase3_reg_id: phase3RegId || undefined,
      }
  ...
  const submitAftercareSection = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        // ensure we link to the same Phase 3 registration
        phase3_reg_id: phase3RegId || undefined,
      }
  ...
  // Load existing Phase 3 data (latest registration if regId not supplied)
  // CHANGE: decrypt response and if latest is complete, prep for new registration instead of hiding UI
  const loadPhase3Data = async (pid: number, regId?: number) => {
    if (!pid) return
    setPhase3Loading(true)
    try {
      const token = sessionStorage.getItem("token") || localStorage.getItem("token")
      const headers = token ? { Authorization: `Bearer ${token}` } : {}
      const res = await api.get(`/api/phase3/resume/${pid}${regId ? `/${regId}` : ""}`, { headers })
      ...
      setPhase3RegId(raw.phase3_reg_id || null)

      // SAFE defaults to avoid runtime errors
      const sections = raw.sections || {}
      const completeness = raw.completeness || {}
      const { registration, earScreening, aftercareAssessment, finalQC } = sections

      // Mark section status
      setSectionSuccess(s => ({
        ...s,
        registration: !!completeness.registration,
        earScreening: !!completeness.earScreening,
        assessmentServices: !!completeness.aftercareAssessment,
        finalQC: !!completeness.finalQC
      }))

      // Helpers
      const flagL = (v: number | null | undefined) => v === 1 || v === 3
      const flagR = (v: number | null | undefined) => v === 2 || v === 3
      ...
      // Prefill only unfinished sections (autofill requirement)
      setFormData(prev => {
        let next = { ...prev }

        // Registration
        if (!completeness.registration && registration) {
          next.phase3_date = registration.registration_date || next.phase3_date
          next.country = registration.country || next.country
          next.phase3_aftercare_city = registration.city || next.phase3_aftercare_city
          next.type_of_aftercare = registration.type_of_aftercare || next.type_of_aftercare
          next.service_center_or_school_name = registration.service_center_school_name || next.service_center_or_school_name
          next.return_visit_custom_earmold_repair = !!registration.return_visit_custom_earmold_repair
          next.problem_with_hearing_aid_earmold = registration.problem_with_hearing_aid_earmold || next.problem_with_hearing_aid_earmold
        }

        // Ear screening
        if (!completeness.earScreening && Array.isArray(earScreening) && earScreening.length) {
          const es = earScreening[0]
          const meds = parsePgTextArray(es.medication_given)
          next.ears_clear_for_assessment = es.ears_clear || next.ears_clear_for_assessment

          next.left_wax = flagL(es.otc_wax)
          next.right_wax = flagR(es.otc_wax)
          next.left_infection = flagL(es.otc_infection)
          next.right_infection = flagR(es.otc_infection)
          next.left_perforation = flagL(es.otc_perforation)
          next.right_perforation = flagR(es.otc_perforation)
          next.left_other = flagL(es.otc_other)
          next.right_other = flagR(es.otc_other)

          next.medical_recommendation = es.medical_recommendation || next.medical_recommendation
          next.left_ear_clear_for_assessment = es.left_ear_clear_for_fitting || next.left_ear_clear_for_assessment
          next.right_ear_clear_for_assessment = es.right_ear_clear_for_fitting || next.right_ear_clear_for_assessment
          next.otoscopy_comments = es.comments || next.otoscopy_comments
          if (meds.length) next.medication_given = meds
        }

        // Aftercare Assessment
        if (!completeness.aftercareAssessment && aftercareAssessment) {
          const a = aftercareAssessment
          const i2b = (v: any) => Number(v) > 0

          next.left_ha_dead_or_broken = flagL(a.eval_hearing_aid_dead_broken)
          next.right_ha_dead_or_broken = flagR(a.eval_hearing_aid_dead_broken)
          next.left_ha_internal_feedback = flagL(a.eval_hearing_aid_internal_feedback)
          next.right_ha_internal_feedback = flagR(a.eval_hearing_aid_internal_feedback)
          next.left_ha_power_change_needed = flagL(a.eval_hearing_aid_power_change_needed)
          next.right_ha_power_change_needed = flagR(a.eval_hearing_aid_power_change_needed)
          next.left_ha_power_change_reason = next.left_ha_power_change_reason
          next.right_ha_power_change_reason = next.right_ha_power_change_reason
          next.left_ha_lost_or_stolen = flagL(a.eval_hearing_aid_lost_stolen)
          next.right_ha_lost_or_stolen = flagR(a.eval_hearing_aid_lost_stolen)
          next.left_ha_no_problem = flagL(a.eval_hearing_aid_no_problem)
          next.right_ha_no_problem = flagR(a.eval_hearing_aid_no_problem)

          next.left_em_discomfort_too_tight = flagL(a.eval_earmold_discomfort_too_tight)
          next.right_em_discomfort_too_tight = flagR(a.eval_earmold_discomfort_too_tight)
          next.left_em_feedback_too_loose = flagL(a.eval_earmold_feedback_too_loose)
          next.right_em_feedback_too_loose = flagR(a.eval_earmold_feedback_too_loose)
          next.left_em_damaged_or_tubing_cracked = flagL(a.eval_earmold_damaged_tubing_cracked)
          next.right_em_damaged_or_tubing_cracked = flagR(a.eval_earmold_damaged_tubing_cracked)
          next.left_em_lost_or_stolen = flagL(a.eval_earmold_lost_stolen)
          next.right_em_lost_or_stolen = flagR(a.eval_earmold_lost_stolen)
          next.left_em_no_problem = flagL(a.eval_earmold_no_problem)
          next.right_em_no_problem = flagR(a.eval_earmold_no_problem)

          next.left_ha_tested_wfa_demo = i2b(a.service_tested_wfa_demo_hearing_aids)
          next.right_ha_tested_wfa_demo = i2b(a.service_tested_wfa_demo_hearing_aids)
          next.left_ha_sent_for_repair_replacement = i2b(a.service_hearing_aid_sent_for_repair_replacement)
          next.right_ha_sent_for_repair_replacement = i2b(a.service_hearing_aid_sent_for_repair_replacement)
          next.left_ha_refit_new = i2b(a.service_refit_new_hearing_aid)
          next.right_ha_refit_new = i2b(a.service_refit_new_hearing_aid)
          next.left_ha_not_benefiting = i2b(a.service_not_benefiting_from_hearing_aid)
          next.right_ha_not_benefiting = i2b(a.service_not_benefiting_from_hearing_aid)
          next.left_em_retubed_unplugged = i2b(a.service_retubed_unplugged_earmold)
          next.right_em_retubed_unplugged = i2b(a.service_retubed_unplugged_earmold)
          next.left_em_modified = i2b(a.service_modified_earmold)
          next.right_em_modified = i2b(a.service_modified_earmold)
          next.left_em_fit_stock = i2b(a.service_fit_stock_earmold)
          next.right_em_fit_stock = i2b(a.service_fit_stock_earmold)
          next.left_em_took_new_impression = i2b(a.service_took_new_ear_impression)
          next.right_em_took_new_impression = i2b(a.service_took_new_ear_impression)
          next.left_em_refit_custom = i2b(a.service_refit_custom_earmold)
          next.right_em_refit_custom = i2b(a.service_refit_custom_earmold)

          next.counseling_provided = a.gs_counseling ?? next.counseling_provided
          next.batteries_provided_13 = a.gs_batteries_13_qty ?? next.batteries_provided_13
          next.batteries_provided_675 = a.gs_batteries_675_qty ?? next.batteries_provided_675
          next.refer_to_aftercare_center = !!a.gs_refer_aftercare_service_center
          next.refer_to_next_phase2_mission = !!a.gs_refer_next_phase2_mission
          next.aftercare_comments = a.comment || next.aftercare_comments
        }

        // Final QC
        if (!completeness.finalQC && finalQC) {
          next.satisfaction_with_hearing = finalQC.hearing_aid_satisfaction_18_plus || next.satisfaction_with_hearing
          next.asks_to_repeat_or_speak_louder = finalQC.ask_people_to_repeat_themselves || next.asks_to_repeat_or_speak_louder
          next.shf_notes = finalQC.notes_from_shf || next.shf_notes
        }

        return next
      })

      // If latest is complete -> proceed to new registration (do not hide form)
      if (Object.values(completeness).every(Boolean)) {
        setLatestRegComplete(true)
        setPhase3RegId(null)
        setSectionSuccess({})
      } else {
        setLatestRegComplete(false)
      }
    } catch (e) {
      console.warn("loadPhase3Data resume failed", e)
    } finally {
      setPhase3Loading(false)
    }
  }

  // NEW: check if patient has ongoing (incomplete) Phase 3
  // CHANGE: decrypt and drive UX: resume if incomplete; otherwise require/register new
  const checkOngoingPhase3 = async (pid: number) => {
    if (!pid) return
    setResumeCheckLoading(true)
    try {
      const token = sessionStorage.getItem("token") || localStorage.getItem("token")
      const headers = token ? { Authorization: `Bearer ${token}` } : {}
      const res = await api.get(`/api/phase3/resume/${pid}`, { headers })

      const payload = res.data?.encrypted_data
        ? decryptObject(res.data.encrypted_data)
        : (res.data?.data ?? res.data)

      const summary = {
        phase3_reg_id: payload?.phase3_reg_id ?? null,
        completeness: payload?.completeness ?? { registration: false, earScreening: false, aftercareAssessment: false, finalQC: false },
        all_complete: !!payload?.all_complete,
      }
      setResumeSummary(summary)

      if (summary.phase3_reg_id && !summary.all_complete) {
        // Incomplete -> ask to resume and then autofill on Continue
        setResumeDialogOpen(true)
        setLatestRegComplete(false)
      } else if (summary.phase3_reg_id && summary.all_complete) {
        // Latest complete -> proceed to new registration
        setResumeDialogOpen(false)
        setLatestRegComplete(true)
        setPhase3RegId(null)
        setSectionSuccess({})
      } else {
        // No registration -> require to register first
        setResumeDialogOpen(false)
        setLatestRegComplete(false)
        setPhase3RegId(null)
        setSectionSuccess({})
      ...
      // Fail silently; require registration
      setResumeDialogOpen(false)
      setLatestRegComplete(false)
      setPhase3RegId(null)
      setSectionSuccess({})
  ...
  useEffect(() => {
    // when patient found and Phase 2 gating passed, check for ongoing Phase 3
    if (patientFound && hasPhase2Record === true) {
      checkOngoingPhase3(getPatientId())
    }
  }, [patientFound, hasPhase2Record])


  const allPhase3Complete = sectionSuccess.registration && sectionSuccess.earScreening && sectionSuccess.assessmentServices && sectionSuccess.finalQC

  const sectionLabel = (k: keyof NonNullable<typeof resumeSummary>["completeness"]) => {
    switch (k) {
      case "registration": return "Registration"
      case "earScreening": return "Ear Screening/Otoscopy"
      case "aftercareAssessment": return "Aftercare Assessment"
      case "finalQC": return "Final Quality Control"
      default: return k
    }
  }
  ...
  return (
    <div className="p-6">
      {/* Require Phase 3 Registration dialog */}
      {needPhase3DialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white w-full max-w-md rounded-lg shadow-lg p-6 space-y-4">
            <h2 className="text-lg font-semibold text-amber-700">Phase 3 Registration Required</h2>
            <p className="text-sm text-gray-700">
              No incomplete Phase 3 registration found. Please complete the Registration section first.
            </p>
            <div className="flex justify-end">
              <button
                type="button"
                className="px-3 py-2 rounded bg-amber-600 hover:bg-amber-700 text-white"
                onClick={() => setNeedPhase3DialogOpen(false)}
              >
                OK
              </button>
            </div>
      ...
      {/* keep existing "complete" banner if you want, but do not hide the form anymore */}
      {showPhase3Complete && (
        <div className="mb-4 p-4 rounded bg-green-50 border border-green-200 text-sm text-green-800">
          Phase 3 is already complete for this registration (ID: {phase3RegId}). You can start a new registration.
        </div>
      )}

      {/* NEW: Ongoing Phase 3 resume dialog */}
      {resumeDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white w-full max-w-md rounded-lg shadow-lg p-6 space-y-4">
            <h2 className="text-lg font-semibold text-blue-700">Ongoing Phase 3 Found</h2>
            <p className="text-sm text-gray-700">
              This patient has an incomplete Phase 3 registration.
              Continue to resume and fill the remaining sections?
            </p>

            <div className="rounded-md border border-gray-200 bg-gray-50 p-3">
              <p className="text-xs text-gray-600 mb-2">Section status:</p>
              <ul className="space-y-1 text-sm">
                <li className={resumeSummary?.completeness.registration ? "text-green-700" : "text-gray-800"}>
                  • Registration — {resumeSummary?.completeness.registration ? "Completed" : "Not completed"}
                </li>
                <li className={resumeSummary?.completeness.earScreening ? "text-green-700" : "text-gray-800"}>
                  • Ear Screening/Otoscopy — {resumeSummary?.completeness.earScreening ? "Completed" : "Not completed"}
                </li>
                <li className={resumeSummary?.completeness.aftercareAssessment ? "text-green-700" : "text-gray-800"}>
                  • Aftercare Assessment — {resumeSummary?.completeness.aftercareAssessment ? "Completed" : "Not completed"}
                </li>
                <li className={resumeSummary?.completeness.finalQC ? "text-green-700" : "text-gray-800"}>
                  • Final Quality Control — {resumeSummary?.completeness.finalQC ? "Completed" : "Not completed"}
                </li>
              </ul>
            </div>

            {resumeCheckLoading && (
              <div className="p-2 text-xs bg-blue-50 border border-blue-200 rounded text-blue-700">
                Checking status...
              </div>
            )}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  const pid = getPatientId()
                  setResumeDialogOpen(false)
                  await loadPhase3Data(pid, resumeSummary?.phase3_reg_id ?? undefined)
                }}
              >
                Continue
              </button>
      ...
      {needPhase2DialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white w-full max-w-md rounded-lg shadow-lg p-6 space-y-4">
            <h2 className="text-lg font-semibold text-amber-700">Phase 2 Required</h2>
            <p className="text-sm text-gray-700">
              This patient has no Phase 2 fitting record. Complete Phase 2 before proceeding to Phase 3.
            </p>
            <div className="flex justify-end">
              <button
                type="button"
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Phase 3 Form</h1>
          </div>
        </div>
      </div>
      {/* Progress Indicator */}
      ...
      {!showPhase3Complete && (
        <ScrollArea className="h-[calc(100vh-180px)] w-full pr-4">
          <div className="space-y-6">
            {/* Patient Search Section (always visible) */}
            ...
            {/* Registration Section (only if incomplete) */}
                ...
                <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                        {["Left", "Right"].map((side) => (
                          <div
                            ...
                            className="bg-white flex items-center space-x-2 p-2 rounded-lg border border-gray-200"
                          >
                  ...
                  <div className="space-y-3 p-4 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray transition-colors shadow-sm">
                    ...
                    <Textarea
                      ...
                      className="bg-white h-40 w-full resize-none overflow-y-auto focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                ...
                <div className="flex justify-end pt-4">
                  <div onClick={() => ensurePatientAndRun(submitEarScreeningSection)} className="inline-block w-full sm:w-auto">
                    <Button
                      ...
                      className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
